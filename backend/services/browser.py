...
class BrowserSessionManager:
    ...
    def _build_locator(cls, target, strategy: str, selector: str):
        ...
        execution (the sidecar's verify handler) so both stay in sync with
        rank_locators' strategy vocabulary.
        """
    ...
    async def _execute_verify_action(cls, locator, action: str, value: Optional[str]):
        ...
        failure — the caller (the sidecar's verify handler) is responsible for catching.
        """
        timeout = 5000
        if action == "click":
    ...
    async def run_page_exploration(cls, session_id: str, goal_prompt: Optional[str] = None, scope: str = "page"):
        ...
        try:
            ...
            # Finalize exactly like the sidecar's scan-page handler: resolve
            # locators for every distinct accumulated item, then name them.
